...
export default function UserModal({ isOpen, onClose, editingUser, rolesList }: UserModalProps) {
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <button type="button" onClick={onClose} disabled={isSubmitting || isResettingPassword} className="px-6 py-3 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors">
            Hủy bỏ thao tác
          </button>
    ...
    >
      <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <form id="user-form" onSubmit={handleSaveUser} className="flex flex-col gap-6">
          ...
          <div className="space-y-1.5 group">
            <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5 group-focus-within:text-indigo-500 transition-colors"><Building2 className="w-3.5 h-3.5"/> Đơn vị / Phòng ban</label>
          ...
          {editingUser && (
            <div className="space-y-1.5 group">
              ...
              <select form="user-form" value={userForm.status} onChange={(e) => setUserForm({...userForm, status: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none text-slate-900 dark:text-white transition-all cursor-pointer">
                ...
                <option value="LOCKED" className="font-bold text-slate-600">🔒 Khóa Bảo mật (Locked)</option>
          ...
          <div className="mt-2 p-5 bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-2xl">
            ...
            {!editingUser ? (
              <div className="flex items-center gap-2 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-white/5">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 font-medium">Hệ thống sẽ cấp tự động: <code className="bg-slate-100 dark:bg-slate-900 px-2 py-1 rounded text-indigo-600 dark:text-indigo-400 font-black tracking-wider shadow-sm ml-1">Password@123</code></p>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {revealedPassword ? (
                  ...
                  <div 
                    ...
                    className="flex flex-col gap-2 p-3 bg-white dark:bg-slate-800 border border-rose-300 dark:border-rose-500/50 rounded-xl shadow-inner"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <input 
                        ...
                        className="flex-1 bg-transparent text-sm outline-none px-2 font-bold tracking-widest text-slate-900 dark:text-white" 
                        autoFocus 
                      />
                      <button 
                        type="button" 
                        ...
                        className="px-4 py-2 bg-rose-600 text-white text-xs font-bold rounded-lg hover:bg-rose-700 disabled:opacity-50"
                      >
                ...
                <p className="text-[10px] text-slate-500 leading-tight">Chỉ Admin hệ thống mới có quyền thao tác thông qua xác thực bảo mật PIN.</p>
              </div>
            )}
          </div>
