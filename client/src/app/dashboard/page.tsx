...
import {
  ...
  WalletCards, AlertOctagon, RefreshCcw, Activity, Clock, Download
} from "lucide-react";
import dayjs from "dayjs";
import 'dayjs/locale/vi';
import { toast } from "react-hot-toast";

// --- GỌI API THẬT TỪ REDUX ---
...
export default function DashboardPage() {
  ...
  // --- HANDLER EXPORT BÁO CÁO ---
  const handleExportReport = () => {
    if (!cashflow || cashflow.length === 0) {
      toast.error("Chưa có dữ liệu dòng tiền để xuất!"); return;
    }
    const exportData = cashflow.map((c: any) => ({
      "Kỳ báo cáo": c.period,
      "Tiền Thu (VND)": c.cashIn,
      "Tiền Chi (VND)": c.cashOut,
      "Dòng tiền Thuần (VND)": c.cashIn - c.cashOut
    }));
    exportToCSV(exportData, "Bao_Cao_Dong_Tien_Dashboard");
  ...
  return (
    <div className="w-full flex flex-col gap-6 pb-10">
      <Header 
        ...
        rightNode={
          <button onClick={handleExportReport} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-bold rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors active:scale-95">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <motion.div variants={itemVariants} className="glass p-5 rounded-2xl border-l-4 border-l-emerald-500 group hover:-translate-y-1 transition-transform duration-300">
            ...
            <div className="flex items-center gap-2 mt-1 pt-2 border-t border-slate-100 dark:border-white/5 text-xs text-slate-400">
              <span className="text-emerald-500 font-medium">In: {(metrics.financials?.totalCashIn || 0).toLocaleString('vi-VN')}</span>
              <span>|</span>
              <span className="text-rose-500 font-medium">Out: {(metrics.financials?.totalCashOut || 0).toLocaleString('vi-VN')}</span>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="glass p-5 rounded-2xl border-l-4 border-l-indigo-500 group hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden">
            ...
            <h3 className="text-3xl font-black text-indigo-600 dark:text-indigo-400 mb-1 relative z-10 truncate">
              {(metrics.tasks?.pendingApprovals || 0).toLocaleString('vi-VN')} <span className="text-sm font-medium text-slate-500">yêu cầu</span>
            </h3>
          </motion.div>
        </div>

        {/* ==========================================
            SECTION 2: BIỂU ĐỒ & HOẠT ĐỘNG
            ========================================== */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <motion.div variants={itemVariants} className="glass p-5 sm:p-6 rounded-2xl flex flex-col h-[400px] sm:h-[446px]">
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
              {metrics.recentActivities && metrics.recentActivities.length > 0 ? (
                metrics.recentActivities.map((log: any) => (
                  <div key={log.logId} className="flex gap-4 group">
                    ...
                    <div className="pb-4">
                      <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 leading-tight">
                        {log.action} <span className="text-blue-600 dark:text-blue-400">{(log as any).tableName || (log as any).module || 'hệ thống'}</span>
                      </p>
                      <div className="flex items-center gap-1.5 mt-1 text-xs text-slate-500">
                        <span className="font-medium text-slate-700 dark:text-slate-400">{log.user?.fullName || "System"}</span>
                        <span>•</span>
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3"/> {dayjs(log.timestamp || log.createdAt).format('HH:mm - DD/MM')}</span>
                      </div>
                    </div>
                  </div>
                ))
