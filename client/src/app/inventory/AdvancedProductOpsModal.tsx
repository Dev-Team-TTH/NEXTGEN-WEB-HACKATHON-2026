...
import { 
  X, Layers, Calendar, AlertTriangle, Plus, Trash2, 
  ArrowRight, Loader2, CheckCircle2, ShieldAlert,
  PackageSearch, Box, Activity, Calculator, History
} from "lucide-react";
...
export default function AdvancedProductOpsModal({ isOpen, onClose, product }: AdvancedProductOpsProps) {
  ...
  useEffect(() => {
    if (isOpen && product) {
      ...
      setFromUomId("");
      setToUomId(product.uomId || ""); // Mặc định đích đến là Đơn vị cơ sở của sản phẩm
      setConversionRate("");
    }
  }, [isOpen, product]);
  ...
  const handleAddBatch = async (e: React.FormEvent) => {
    ...
    try {
      await createBatch({
        ...
        batchNumber,
        manufacturingDate: new Date(mfgDate).toISOString(),
  ...
  // Dữ liệu an toàn hiển thị Tên sản phẩm
      ...
      {isOpen && product && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl bg-slate-50 dark:bg-[#0B0F19] rounded-3xl shadow-2xl border border-indigo-500/30 overflow-hidden z-10 flex flex-col md:flex-row h-[85vh]"
          >
            {/* ========================================== */}
                      ...
                      <div className="flex-1 flex flex-col min-w-0">
                        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-white/10 shadow-sm p-5 flex-1 flex flex-col">
                          ...
                          {loadingBatches ? (
                            ...
                            <motion.div variants={listVariants} initial="hidden" animate="visible" className="flex flex-col gap-3 overflow-y-auto pr-2">
