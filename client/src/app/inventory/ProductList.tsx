...
import { 
  ...
  Tag, ShieldAlert, CheckCircle2, Loader2, Download, UploadCloud, FileText, X
} from "lucide-react";
  ...
  const columns: ColumnDef<Product>[] = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        const uomName = uoms.find((u: any) => u.uomId === row.uomId)?.name || "N/A";
        return (
    ...
    <div className="w-full flex flex-col gap-6 relative">
      ...
      {isLoading && products.length === 0 ? <ProductListSkeleton /> : (
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="flex flex-col gap-5 w-full">
          ...
          {/* THANH TÌM KIẾM & LỌC (Kết nối API) */}
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center justify-end gap-4 bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm z-10">
          ...
          {/* BẢNG DỮ LIỆU ĐỘNG - SERVER SIDE PAGINATION */}
