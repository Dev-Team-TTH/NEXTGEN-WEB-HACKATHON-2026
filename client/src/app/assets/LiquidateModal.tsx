...
import { 
  X, ShieldAlert, Loader2, CheckCircle2, 
...
export default function LiquidateModal({ assetId, isOpen, onClose }: LiquidateModalProps) {
  ...
  // --- HELPER & DATA VIZ CALCULATION ---
  const formatVND = (val: number) => new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(val);
  
  // Tính toán Lãi / Lỗ (P&L) khi thanh lý
  const currentValue = asset?.currentValue || 0;
  const recoverValue = Number(liquidationValue) || 0;
  const netProfitLoss = recoverValue - currentValue; // Nếu bán được nhiều hơn giá trị sổ sách -> Lãi. Ngược lại -> Lỗ.
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="relative w-full max-w-2xl glass-panel rounded-3xl shadow-2xl border border-rose-500/20 overflow-hidden z-10 flex flex-col max-h-[90vh]"
          >
            {/* Header (Tone màu Hủy bỏ / Nguy hiểm) */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-rose-500/10 bg-rose-50/50 dark:bg-rose-900/10 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-rose-700 dark:text-rose-400 leading-tight">Thanh lý Tài sản (Liquidation)</h2>
                  <p className="text-xs text-rose-600/70 font-medium">Ghi giảm tài sản, hủy bỏ hoặc bán phế liệu</p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto scrollbar-thin bg-white/50 dark:bg-black/20 flex flex-col gap-6">
              ...
              {isLoadingAsset ? (
                ...
                <>
                  ...
                  {/* FORM NHẬP LIỆU */}
                  <form id="liquidate-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {/* Ngày thanh lý */}
                      <div className="space-y-1.5">
                        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                          <CalendarDays className="w-4 h-4 text-slate-400" /> Ngày ghi nhận <span className="text-rose-500">*</span>
                        </label>
                        <input 
                          type="date" 
                          value={liquidationDate}
                          onChange={(e) => setLiquidationDate(e.target.value)}
                          className="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-sm outline-none text-slate-900 dark:text-white focus:ring-2 focus:ring-rose-500"
                        />
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                          <DollarSign className="w-4 h-4 text-emerald-500" /> Tiền thu hồi được (VND) <span className="text-rose-500">*</span>
                        </label>
                        <input 
                          type="number" 
                          ...
                          min="0"
                          placeholder="0 nếu hỏng vứt đi"
                          className="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-bold outline-none text-slate-900 dark:text-white focus:ring-2 focus:ring-rose-500"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                        <FileText className="w-4 h-4 text-slate-400" /> Lý do thanh lý / Tình trạng <span className="text-rose-500">*</span>
                      </label>
                      <textarea
                        value={reason}
                        ...
                        rows={3}
                        className="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-sm outline-none text-slate-900 dark:text-white resize-none focus:ring-2 focus:ring-rose-500"
                      />
                    </div>
