    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 transition-all overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden my-auto flex flex-col max-h-[90vh]">
        ...
        <form id="editAssetForm" onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-5 p-5 bg-orange-50 border border-orange-200 rounded-xl shadow-sm">
            <div>
              ...
              <input type="number" min="0" name="depreciationMonths" value={formData.depreciationMonths} onChange={handleChange} className="w-full px-4 py-2.5 rounded-xl border border-orange-300 focus:ring-2 focus:ring-orange-500 outline-none font-medium bg-white" disabled={isSubmitting} />
            </div>
            <div>
              <label className="block text-sm font-bold text-orange-800 mb-1.5 flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4"/> Chu kỳ bảo trì (Tháng)
              </label>
              <input type="number" min="0" name="maintenanceCycle" value={formData.maintenanceCycle} onChange={handleChange} className="w-full px-4 py-2.5 rounded-xl border border-orange-300 focus:ring-2 focus:ring-orange-500 outline-none font-medium bg-white" disabled={isSubmitting} />
            </div>
          </div>
