...
export default function HandoverModal({ assetId, isOpen, onClose }: HandoverModalProps) {
  ...
  const { data: history = [], isLoading: isLoadingHistory } = useGetAssetHistoryQuery(assetId || "", { skip: !assetId || !isOpen || asset?.status !== "IN_USE" });
    ...
    <>
      ...
      {(asset?.status === "ACTIVE" || asset?.status === "IN_USE") && (
        <button 
          ...
          className={cn(
            "flex items-center gap-2 px-6 py-2.5 text-white text-sm font-bold rounded-xl shadow-lg transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed",
        ...
        >
          {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Đang xử lý...</> : <><CheckCircle2 className="w-4 h-4" /> Xác nhận {isHandoverMode ? "Bàn giao" : "Thu hồi"}</>}
        </button>
      )}
    </>
  );
