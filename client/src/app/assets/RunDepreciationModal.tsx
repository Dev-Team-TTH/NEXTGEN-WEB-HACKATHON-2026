    ...
    if (window.confirm(`XÁC NHẬN CHẠY KHẤU HAO [${periodName}]\n\nHệ thống sẽ tự động trừ giá trị tài sản và sinh bút toán kế toán. Thao tác này không thể hoàn tác. Bạn có chắc chắn?`)) {
      try {
        await runDepreciation({ 
          ...
          description: `Khấu hao tài sản tự động kỳ ${periodName}`
        }).unwrap();
  ...
  return (
    ...
    >
      <div className="p-6 sm:p-8 flex flex-col gap-6">
          ...
          {processSteps.map((step, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col items-center text-center gap-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-white/5 relative">
                ...
                {/* Mũi tên kết nối */}
                {idx < 2 && (
                  <div className="absolute top-1/2 -translate-y-1/2 right-[-10px] sm:right-[-12px] text-slate-300 dark:text-slate-600 hidden xs:block z-10">
                      ▶
                  </div>
                )}
              </div>
        ...
        <div className="space-y-2 mt-2 group">
          <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2 group-focus-within:text-purple-500 transition-colors">
            <CalendarDays className="w-4 h-4" /> Chọn Kỳ Kế toán (Fiscal Period) <span className="text-rose-500">*</span>
          </label>
          
          {!activeBranchId ? (
             <div className="w-full px-4 py-3 bg-rose-50 border border-rose-200 rounded-xl text-sm font-semibold text-rose-600 text-center">
                Vui lòng chọn Chi nhánh làm việc trên thanh Header!
             </div>
          ) : loadingPeriods ? (
            <div className="w-full h-[52px] bg-slate-100 dark:bg-slate-800 rounded-xl animate-pulse" />
          ) : (
            <select 
              value={fiscalPeriodId}
              onChange={(e) => setFiscalPeriodId(e.target.value)}
              disabled={isRunning || fiscalPeriods.length === 0}
              className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-bold focus:ring-2 focus:ring-purple-500 outline-none text-slate-900 dark:text-white disabled:opacity-50 shadow-sm cursor-pointer" 
            >
