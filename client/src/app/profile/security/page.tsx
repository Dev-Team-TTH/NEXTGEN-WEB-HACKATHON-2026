...
import { 
  ...
  Sparkles, Fingerprint, LogOut, QrCode // ĐÃ FIX: Thêm QrCode
} from "lucide-react";
    ...
    <div className="w-full max-w-7xl mx-auto pb-24 px-4 sm:px-6 lg:px-8 mt-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 sm:gap-8">
          ...
          <div className="bg-white/70 dark:bg-[#0B0F19]/70 backdrop-blur-2xl border border-slate-200/50 dark:border-white/10 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.04)] p-6 sm:p-8 h-full flex flex-col">
            ...
            <div className="flex flex-col gap-5 flex-1 justify-center">
              <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                  ...
                  <input type="password" placeholder="••••••••" className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-black/40 border border-slate-200/80 dark:border-white/10 rounded-2xl text-sm font-bold text-slate-900 dark:text-white outline-none focus:bg-white dark:focus:bg-[#0B0F19] focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                </div>
              </div>
              <div className="flex flex-col gap-2">
            ...
            <div className="mt-8 pt-6 border-t border-slate-200/50 dark:border-white/5">
              <button className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-100 text-white dark:text-slate-900 rounded-2xl font-black text-sm shadow-[0_8px_20px_rgba(0,0,0,0.1)] transition-all duration-300 active:scale-95 transform-gpu">
                Cập nhật Mật khẩu
              </button>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6 sm:gap-8">
          ...
          <div className="relative bg-white/70 dark:bg-[#0B0F19]/70 backdrop-blur-2xl border border-slate-200/50 dark:border-white/10 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.04)] p-6 sm:p-8 transform-gpu overflow-hidden">
            ...
            <div className="flex justify-between items-start relative z-10 mb-2">
              ...
              <button 
                ...
                className={`relative inline-flex h-8 w-14 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors duration-300 ease-in-out focus:outline-none shadow-inner ${is2FAEnabled ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}
              >
                <span className={`pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow-md ring-0 transition duration-300 ease-in-out ${is2FAEnabled ? 'translate-x-7' : 'translate-x-0.5'}`} />
              </button>
            </div>

            {/* Khung Hướng dẫn 2FA Mở Rộng */}
            <div className={`transition-all duration-500 overflow-hidden relative z-10 ${is2FAEnabled ? "max-h-[500px] opacity-100 mt-8" : "max-h-0 opacity-0 mt-0"}`}>
              <div className="p-6 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-200/60 dark:border-emerald-500/20 rounded-3xl flex flex-col sm:flex-row gap-8 items-center relative overflow-hidden">
                {/* Vi mạch trang trí chìm */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-[url('/noise.png')] opacity-20 mix-blend-overlay pointer-events-none" />

                {/* Khung Mã QR Thẩm Mỹ */}
                <div className="w-40 h-40 bg-white rounded-2xl shadow-[0_8px_32px_rgba(16,185,129,0.2)] border-2 border-emerald-100 flex items-center justify-center shrink-0 relative p-2">
                  <div className="w-full h-full border border-dashed border-emerald-300 rounded-xl flex items-center justify-center bg-emerald-50 text-center flex-col">
                    <QrCode className="w-10 h-10 text-emerald-500 mb-2" />
                    <span className="text-[10px] font-black text-emerald-700 tracking-widest uppercase">Mã QR</span>
                  </div>
                </div>
                ...
                <div className="relative z-10 flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input type="text" placeholder="000 000" maxLength={6} className="w-full sm:w-40 px-5 py-3.5 bg-white dark:bg-black/40 border border-emerald-200 dark:border-emerald-500/30 rounded-xl text-center font-black text-xl tracking-[0.3em] text-emerald-900 dark:text-emerald-400 outline-none focus:ring-2 focus:ring-emerald-500/50 shadow-inner" />
                    <button className="w-full sm:w-auto px-6 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl shadow-[0_8px_20px_rgba(16,185,129,0.3)] transition-all active:scale-95">Xác minh mã</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/70 dark:bg-[#0B0F19]/70 backdrop-blur-2xl border border-slate-200/50 dark:border-white/10 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.04)] p-6 sm:p-8 transform-gpu">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-5 p-5 bg-blue-50/50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/20 rounded-3xl relative overflow-hidden">
                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-500 rounded-l-3xl" />
                <div className="w-12 h-12 rounded-2xl bg-white dark:bg-black/20 shadow-sm border border-slate-100 dark:border-white/5 flex items-center justify-center shrink-0">
                  <Monitor className="w-6 h-6 text-blue-600 dark:text-blue-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-extrabold text-slate-900 dark:text-white flex items-center gap-2 truncate">
                    Windows 11 - Google Chrome 
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-black bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 uppercase tracking-widest shrink-0">Hiện tại</span>
                  </p>
                  <p className="text-xs font-semibold text-slate-500 mt-1 truncate">TP. Hồ Chí Minh, VN • IP: 113.168.12.112</p>
                </div>
              </div>

              {/* Thiết bị cũ (Điện thoại) */}
              <div className="flex items-center gap-5 p-5 border border-slate-100 dark:border-white/5 rounded-3xl hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-extrabold text-slate-700 dark:text-slate-300 truncate">iPhone 14 Pro Max - Safari</p>
                  <p className="text-xs font-semibold text-slate-500 mt-1 truncate">Hà Nội, VN • IP: 14.232.1.200 • Đăng nhập 2 ngày trước</p>
                </div>
                <button className="p-3 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:shadow-sm rounded-xl transition-all active:scale-90" title="Đăng xuất thiết bị này">
                  <LogOut className="w-5 h-5" />
                </button>
              </div>
            </div>
            
            <button className="mt-8 w-full py-4 border border-rose-200 dark:border-rose-500/20 rounded-2xl text-sm font-black text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors">
