    ...
    <div className="w-full flex flex-col gap-6 pb-10 max-w-6xl mx-auto">
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          {/* Card Lịch sử Đăng nhập */}
          <motion.div variants={itemVariants} className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1 flex items-center gap-2">
              <History className="w-5 h-5 text-purple-500" /> Lịch sử Hoạt động (10 lần gần nhất)
            </h3>
            <p className="text-sm text-slate-500 mb-6">Theo dõi các thiết bị và IP đã truy cập vào tài khoản của bạn.</p>
            ...
            {loadingHistory ? (
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead>
                    <tr className="text-slate-500 border-b border-slate-200 dark:border-white/10">
                      ...
                      <th className="pb-3 font-semibold">IP / Vị trí</th>
                      <th className="pb-3 font-semibold">Thiết bị / Trình duyệt</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                    {loginHistory.map((log) => (
                      <tr key={log.id} className="text-slate-700 dark:text-slate-300">
                        <td className="py-3">
                          {new Date(log.timestamp).toLocaleString('vi-VN', { hour: '2-digit', minute:'2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })}
                        </td>
                        <td className="py-3 flex items-center gap-2">
                          <Globe className="w-4 h-4 text-slate-400" /> {log.ipAddress || "Không rõ"}
                        </td>
                        <td className="py-3 max-w-[200px] truncate" title={log.userAgent}>
                          <div className="flex items-center gap-2">
                            <MonitorSmartphone className="w-4 h-4 text-slate-400 flex-shrink-0" />
                            <span className="truncate">{log.userAgent ? log.userAgent.split(' ')[0] : "Không rõ thiết bị"}</span>
                          </div>
                        </td>
                        <td className="py-3">
                          {log.status.includes('SUCCESS') ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 text-xs font-bold">
                              <CheckCircle2 className="w-3.5 h-3.5" /> Thành công
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-rose-50 dark:bg-rose-500/10 text-rose-600 text-xs font-bold" title={log.status}>
              ...
              <div className="text-center py-6 text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
